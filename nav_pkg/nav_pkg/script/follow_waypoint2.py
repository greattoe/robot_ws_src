import rclpy
from rclpy.node import Node, sys
...
def main(args=None):	
    ...
    rgoal.pose.position.x = 1.4
    rgoal.pose.position.y = -1.35
    rgoal.pose.orientation.w = 0.0
    
    q = quaternion_from_euler(radians(270), 0.0, 0.0, 'rzyx')
    
    rgoal.pose.orientation.x = q[0]
    rgoal.pose.orientation.y = q[1]
    rgoal.pose.orientation.z = q[2]
    rgoal.pose.orientation.w = q[3]
    print(rgoal)
    mgoal = [rgoal]
